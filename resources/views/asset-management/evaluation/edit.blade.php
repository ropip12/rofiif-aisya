...
@section('title', 'Edit Evaluasi Aset')
@section('page_title', 'Edit Evaluasi Aset')

@section('content')
    <div class="page-card">
        <h1>Edit Data Evaluasi</h1>
        <div class="subtitle">Perbarui hasil evaluasi aset.</div>

        <form method="POST" action="{{ route('aset.evaluasi.update', ['asset_evaluation' => $item->id]) }}" style="display:grid; gap:16px; max-width:800px;">
            @csrf
            @method('PUT')
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap:16px;">
                <div>
                    ...
                    <input type="date" id="evaluation_date" name="evaluation_date" value="{{ old('evaluation_date', $item->evaluation_date?->format('Y-m-d')) }}" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
                ...
                <div>
                    ...
                    <input type="text" id="condition" name="condition" value="{{ old('condition', $item->condition) }}" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap:16px;">
                <div>
                    ...
                    <input type="text" id="evaluation_result" name="evaluation_result" value="{{ old('evaluation_result', $item->evaluation_result) }}" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
                ...
                <div>
                    ...
                    <input type="text" id="follow_up_status" name="follow_up_status" value="{{ old('follow_up_status', $item->follow_up_status) }}" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
            ...
            <div>
                ...
                <textarea id="recommendation" name="recommendation" rows="3" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">{{ old('recommendation', $item->recommendation) }}</textarea>
            ...
            <div style="display:flex; gap:12px; flex-wrap:wrap;">
                ...
                <a href="{{ route('aset.evaluasi.index') }}" style="background:#e2e8f0; color:#0f172a; padding:11px 16px; border-radius:8px; font-weight:700;">Kembali</a>
            </div>
        </form>
    </div>
@endsection
