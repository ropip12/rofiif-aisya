...
<div class="page-card">
    ...
    <form method="POST" action="{{ route('risiko.store') }}" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap:18px;">
        @csrf

        <div>
            <label for="risk_code" style="display:block; margin-bottom:6px; font-weight:700;">Kode Risiko</label>
            <input type="text" id="risk_code" name="risk_code" value="{{ old('risk_code') }}" required style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
        </div>

        <div>
            <label for="risk_name" style="display:block; margin-bottom:6px; font-weight:700;">Nama Risiko</label>
            <input type="text" id="risk_name" name="risk_name" value="{{ old('risk_name') }}" required style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
        </div>

        <div style="grid-column:1 / -1;">
            <label for="cause" style="display:block; margin-bottom:6px; font-weight:700;">Penyebab</label>
            <textarea id="cause" name="cause" rows="3" required style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">{{ old('cause') }}</textarea>
        </div>

        <div style="grid-column:1 / -1;">
            <label for="impact" style="display:block; margin-bottom:6px; font-weight:700;">Dampak</label>
            <textarea id="impact" name="impact" rows="3" required style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">{{ old('impact') }}</textarea>
        </div>

        <div>
            <label for="likelihood" style="display:block; margin-bottom:6px; font-weight:700;">Kemungkinan</label>
            <input type="text" id="likelihood" name="likelihood" value="{{ old('likelihood') }}" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
        </div>
        ...
        <div>
            ...
            <select id="risk_level" name="risk_level" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
                ...
                @foreach ($riskLevels as $level)
                    <option value="{{ $level }}" {{ old('risk_level') == $level ? 'selected' : '' }}>{{ $level }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select id="risk_status" name="risk_status" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">
                ...
                @foreach ($riskStatuses as $status)
                    <option value="{{ $status }}" {{ old('risk_status') == $status ? 'selected' : '' }}>{{ $status }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div style="grid-column:1 / -1;">
            ...
            <textarea id="control_measures" name="control_measures" rows="3" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">{{ old('control_measures') }}</textarea>
        </div>

        <div style="grid-column:1 / -1;">
            <label for="mitigation_plan" style="display:block; margin-bottom:6px; font-weight:700;">Rencana Mitigasi</label>
            <textarea id="mitigation_plan" name="mitigation_plan" rows="3" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">{{ old('mitigation_plan') }}</textarea>
        </div>

        <div style="grid-column:1 / -1;">
            <label style="display:block; margin-bottom:6px; font-weight:700;">Aset terkait</label>
            @if ($assets->isEmpty())
                <div class="muted">Tidak ada aset tersedia.</div>
            @else
        ...
        <div style="grid-column:1 / -1;">
            ...
            @if ($services->isEmpty())
                <div class="muted">Tidak ada layanan tersedia.</div>
            @else
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:10px;">
                    @foreach ($services as $service)
                        <label style="display:flex; align-items:center; gap:8px; border:1px solid #dfe7f4; border-radius:8px; padding:10px 12px;">
                            <input type="checkbox" name="services[]" value="{{ $service->id }}" {{ in_array($service->id, old('services', []), true) ? 'checked' : '' }}>
        ...
        <div style="grid-column:1 / -1;">
            ...
            <textarea id="monitoring" name="monitoring" rows="2" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">{{ old('monitoring') }}</textarea>
        </div>

        <div style="grid-column:1 / -1;">
            <label for="evaluation" style="display:block; margin-bottom:6px; font-weight:700;">Evaluasi</label>
            <textarea id="evaluation" name="evaluation" rows="2" style="width:100%; padding:10px 12px; border:1px solid #dfe7f4; border-radius:8px;">{{ old('evaluation') }}</textarea>
        </div>
        ...
        <div style="grid-column:1 / -1; display:flex; gap:12px; flex-wrap:wrap; margin-top:8px;">
            ...
            <a href="{{ route('risiko.index') }}" style="background:#e5e7eb; color:#111827; padding:12px 18px; border-radius:8px; font-weight:700; text-decoration:none;">Batal</a>
        </div>
    </form>
</div>
@endsection
